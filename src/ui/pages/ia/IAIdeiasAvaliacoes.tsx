...
import { ArrowLeft, Sparkles, Loader2, RefreshCw, Printer, FileText, Copy, Check, ImagePlus } from 'lucide-react';
...
// Reduz a imagem gerada (vem grande) pra ~800px: deixa o Word e a memória da
// tela leves, sem perder qualidade útil numa caixa de 200px.
function reduzirImagem(dataUrl: string, larguraMax = 800): Promise<string> {
...
interface ErroImagem extends Error { fatal?: boolean }

// Uma imagem por chamada. Repete se a OpenAI limitar as imagens por minuto (429);
// erros que se repetiriam em todas as questões (login, chave, saldo, verificação
// da organização) são marcados como "fatal" pra o lote parar em vez de insistir.
async function gerarImagemNoServidor(prompt: string): Promise<string> {
  ...
  for (let tentativa = 0; tentativa < 3; tentativa++) {
    ...
    if (resp.ok && dados.base64) return reduzirImagem(`data:${dados.contentType || 'image/jpeg'};base64,${dados.base64}`);

    const semSaldo = dados.codigo === 'insufficient_quota' || dados.codigo === 'billing_hard_limit_reached';
    if (resp.status === 429 && !semSaldo && tentativa < 2) {
    ...
    erro.fatal = [401, 402, 403].includes(resp.status) || semSaldo || dados.codigo === 'sem_chave';
    throw erro;
  }
...
export function IAIdeiasAvaliacoes() {
  ...
  async function gerarImagens(somenteQuestao?: number) {
    ...
    for (const q of alvo) {
      try {
        const url = await gerarImagemNoServidor(promptImagem(q));
            ...
            {(progressoImagens !== null || questoes.some(q => !q.imagemDataUrl)) && (
              ...
              >
                ...
                {progressoImagens
                  ...
                  : questoes.some(q => q.imagemDataUrl) ? 'Gerar as imagens que faltam' : 'Gerar imagens com IA'}
              </button>
            )}
            <p className="text-[11px] text-on-surface-variant">
              Uma imagem por questão, gerada uma de cada vez (cerca de 20 a 40 segundos cada) — não feche a tela durante a geração.
              Custa uns poucos centavos por imagem na sua conta da OpenAI. Se alguma sair errada, refaça só ela.
            </p>
            ...
            <div key={q.numero} className="bg-surface border border-outline-variant rounded-2xl overflow-hidden">
              ...
              <div className="p-4 space-y-3">
                {q.imagemDataUrl ? (
                  ...
                  <div className="w-full h-24 rounded-xl border-2 border-dashed border-outline-variant flex flex-col items-center justify-center gap-1.5 text-center px-3">
                    ...
                    >
                      <ImagePlus className="w-3.5 h-3.5" /> Gerar imagem desta questão
                    </button>
                  </div>
                )}
