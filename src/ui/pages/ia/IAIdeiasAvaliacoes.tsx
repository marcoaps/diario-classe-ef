...
import { htmlImpressao, htmlWord, type DadosProva } from './provaAdaptadaHtml';
...
interface Questao {
  ...
  /** Crédito da imagem vinda do banco (pictograma/foto com licença); sai no rodapé da prova. */
...
const CUIDADO_IMAGEM_POR_NEE: Record<string, string> = {
  'Autismo (TEA)': 'poucos elementos, fundo liso sem detalhes que distraiam, representação literal e realista',
  'TDAH': 'poucos elementos e fundo liso, sem detalhes que distraiam',
  'Deficiencia Visual': 'alto contraste, formas grandes e contornos bem marcados',
  'Deficiencia Intelectual (DI)': 'objetos e ações concretos e fáceis de reconhecer, sem elementos abstratos',
  'Deficiencia Multipla': 'objetos concretos, formas grandes, alto contraste e fundo liso',
};

// Prompt completo pra colar no gerador de imagens (ex: Mídia Mágica do Canva).
...
- Não escreva texto, letras, números, placas, faixas nem bandeiras dentro das imagens: nenhuma palavra ou nome de lugar deve aparecer desenhado.
...
export function IAIdeiasAvaliacoes() {
  ...
  // que o aluno lê, as alternativas e a resposta correta (não só o assunto).
  function contextoDaImagem(q: Questao): ContextoImagem {
    return {
      disciplina: 'Educação Física',
      serie,
      tema,
      enunciado: [q.contexto, q.pergunta].filter(Boolean).join(' '),
      alternativas: [
  ...
  // Sem `somenteQuestao`: gera as que ainda não têm imagem, uma de cada vez.
  // Com `somenteQuestao`: gera (ou refaz) só essa.
  async function gerarImagens(somenteQuestao?: number) {
    const alvo = questoes.filter(q => (somenteQuestao ? q.numero === somenteQuestao : !q.imagemDataUrl));
    ...
    for (const q of alvo) {
      try {
        const url = await gerarImagemDaQuestao(contextoDaImagem(q));
        setQuestoes(prev => prev.map(x => (x.numero === q.numero ? { ...x, imagemDataUrl: url, imagemCredito: undefined } : x)));
      } catch (e) {
  ...
  function removerImagem(numero: number) {
    setQuestoes(prev => prev.map(x => (x.numero === numero ? { ...x, imagemDataUrl: undefined, imagemCredito: undefined } : x)));
  }

  // Imagem que o professor já tem pronta (ex.: gerada no chat do Gemini): enviada
  // como arquivo, arrastada, colada com Ctrl+V ou colada pelo botão.
  async function usarImagemManual(numero: number, arquivo: Blob | null | undefined) {
    ...
    try {
      ...
      setQuestoes(prev => prev.map(x => (x.numero === numero ? { ...x, imagemDataUrl: url, imagemCredito: undefined } : x)));
    } catch (e) {
  ...
  async function enviarVariasImagens(lista: FileList | null) {
    ...
    setQuestoes(prev => prev.map(x => (novas.has(x.numero) ? { ...x, imagemDataUrl: novas.get(x.numero), imagemCredito: undefined } : x)));
    const partes: string[] = [];
  ...
  // Imagem do banco (pictograma ou foto aprovada): entra igual às outras, mas guarda o
  // crédito exigido pela licença para sair no rodapé da prova.
  async function carregarItemDoBanco(item: ItemBancoImagem): Promise<string> {
  ...
  async function usarItemDoBanco(numero: number, item: ItemBancoImagem) {
    setBancoAbertoPara(null);
    setErroImagens('');
    setAvisoImagens('');
    try {
      const url = await carregarItemDoBanco(item);
      setQuestoes(prev => prev.map(x => (x.numero === numero ? { ...x, imagemDataUrl: url, imagemCredito: item.credito } : x)));
    } catch (e) {
      setErroImagens(`Questão ${numero}: ${(e as Error).message}`);
    }
  }
  ...
  async function aplicarSugestoesDoBanco(lista: Questao[] = questoes, automatico = false) {
    ...
    const resultados = await Promise.allSettled(alvo.map(x => carregarItemDoBanco(x.item)));
    const novas = new Map<number, { url: string; credito: string }>();
    const erros: string[] = [];
    resultados.forEach((r, i) => {
      if (r.status === 'fulfilled') novas.set(alvo[i].q.numero, { url: r.value, credito: alvo[i].item.credito });
      else erros.push(`Questão ${alvo[i].q.numero}: ${(r.reason as Error).message}`);
    });
    setQuestoes(prev => prev.map(x => {
  ...
  function arquivoDeImagem(dados: DataTransfer | null): File | null {
    return Array.from(dados?.files ?? []).find(f => f.type.startsWith('image/')) ?? null;
  }

  async function colarImagemDaAreaDeTransferencia(numero: number) {
    try {
      const itens = await navigator.clipboard.read();
      for (const item of itens) {
        const tipo = item.types.find(t => t.startsWith('image/'));
        if (tipo) {
          await usarImagemManual(numero, await item.getType(tipo));
          return;
        }
      }
      setErroImagens(`Questão ${numero}: não há imagem copiada. Copie a imagem no Gemini (botão direito → Copiar imagem) e tente de novo.`);
    } catch {
      setErroImagens(`Questão ${numero}: o navegador não deixou ler a área de transferência. Clique na caixa da imagem e aperte Ctrl+V, ou use "Enviar imagem".`);
    }
  }

  function botoesImagemManual(numero: number) {
    const desabilitado = progressoImagens !== null;
    return (
      <>
        <label className={`flex items-center gap-1 px-2.5 py-1 rounded-lg bg-primary text-on-primary text-xs font-semibold cursor-pointer ${desabilitado ? 'opacity-50 pointer-events-none' : ''}`}>
          ...
          <input
            ...
            onChange={e => {
              ...
              e.target.value = '';
              usarImagemManual(numero, arquivo);
            }}
          />
        </label>
        <button
          onClick={() => setBancoAbertoPara(numero)}
        ...
        <button
          onClick={() => colarImagemDaAreaDeTransferencia(numero)}
          disabled={desabilitado}
          className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-secondary-container text-on-secondary-container text-xs font-semibold disabled:opacity-50"
        >
          <Clipboard className="w-3.5 h-3.5" /> Colar imagem
        </button>
      </>
    );
  }
  ...
  function dadosDaProva(): DadosProva {
    return { questoes, serie, turma, alunoNumero, nomeAluno: alunoNome || '____________________________________________', logoSrc: LOGO_IOP };
  }

  // A prova do aluno é uma página só (ver provaAdaptadaHtml.ts): cabeçalho pequeno, questões
  // numeradas só com "1.", "2.", ... e os créditos das imagens no rodapé.
  function imprimir() {
    // Sem gabarito na impressão do aluno
    const blob = new Blob([htmlImpressao(dadosDaProva())], { type: 'text/html;charset=utf-8' });
    window.open(URL.createObjectURL(blob), '_blank');
  }

  function exportarWord() {
    // Página 1 = prova do aluno; página 2 = gabarito e guia de imagens (para o professor).
  ...
  return (
    <div className="py-4 space-y-4">
      {bancoAbertoPara !== null && (() => {
        const q = questoes.find(x => x.numero === bancoAbertoPara);
        return (
          <SeletorBancoImagens
            tituloQuestao={q?.titulo}
            textoQuestao={[q?.titulo, q?.contexto, q?.pergunta, q?.imageQuery].filter(Boolean).join(' ')}
            onEscolher={item => usarItemDoBanco(bancoAbertoPara, item)}
            onFechar={() => setBancoAbertoPara(null)}
          />
        );
      })()}
      ...
      {questoes.length > 0 && (
        <div className="space-y-4">
          ...
          <div className="bg-surface border border-outline-variant rounded-2xl p-3 space-y-2">
            <p className="text-xs font-semibold text-on-surface">Imagens das questões</p>
            ...
            {(progressoImagens !== null || questoes.some(q => !q.imagemDataUrl)) && (
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-2 rounded-xl bg-surface-variant text-on-surface-variant text-xs font-semibold disabled:opacity-60"
              >
            ...
            <div key={q.numero} className="bg-surface border border-outline-variant rounded-2xl overflow-hidden">
              ...
              <div className="p-4 space-y-3">
                <div
                  tabIndex={0}
                  onPaste={e => {
                    const arquivo = arquivoDeImagem(e.clipboardData);
                    if (arquivo) { e.preventDefault(); usarImagemManual(q.numero, arquivo); }
                  }}
                  onDragOver={e => e.preventDefault()}
                  onDrop={e => {
                    const arquivo = arquivoDeImagem(e.dataTransfer);
                    if (arquivo) { e.preventDefault(); usarImagemManual(q.numero, arquivo); }
                  }}
                ...
                >
                  {q.imagemDataUrl ? (
                    <div className="space-y-1.5">
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        <button
                          ...
                          disabled={progressoImagens !== null}
                          className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-surface-variant text-on-surface-variant text-xs font-semibold disabled:opacity-50"
                        >
                          <RefreshCw className="w-3.5 h-3.5" /> Gerar outra (IA)
                        </button>
                        <button
                          onClick={() => removerImagem(q.numero)}
                          disabled={progressoImagens !== null}
                    ...
                    <div className="w-full rounded-xl border-2 border-dashed border-outline-variant flex flex-col items-center justify-center gap-1.5 text-center px-3 py-3">
                          ...
                          <div className="flex flex-wrap items-center justify-center gap-2">
                            ...
                            {sugerido && (
                              <button
                                ...
                                disabled={progressoImagens !== null}
                                className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-primary text-on-primary text-xs font-semibold disabled:opacity-50"
                              >
                      ...
                      <div className="flex flex-wrap justify-center gap-2">
                        ...
                        <button
                          ...
                          className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-surface-variant text-on-surface-variant text-xs font-semibold disabled:opacity-50"
                        >
                          <ImagePlus className="w-3.5 h-3.5" /> Gerar com IA
                        </button>
                      </div>
                    </div>
                  )}
            ...
            {questoes.filter(q => tipoDaQuestao(q) === 'ilustracao').map(q => (
              <div key={q.numero} className="space-y-1">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <button
                    onClick={() => copiar(String(q.numero), promptImagem(q))}
                    className="shrink-0 flex items-center gap-1 px-2.5 py-1 rounded-lg bg-surface-variant text-on-surface-variant text-xs font-semibold"
                  >
                    {copiado === String(q.numero) ? <Check className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />}
                    {copiado === String(q.numero) ? 'Copiado!' : 'Copiar prompt'}
                  </button>
                </div>
                <p className="text-xs text-on-surface-variant leading-snug">{promptImagem(q)}</p>
              </div>
            ))}
          </div>
        </div>
