...
import { ArrowLeft, Sparkles, Loader2, RefreshCw, Printer, FileText, Copy, Check, ImagePlus, Upload, Clipboard } from 'lucide-react';
...
export function IAIdeiasAvaliacoes() {
  ...
  async function gerarImagens(somenteQuestao?: number) {
    ...
    for (const q of alvo) {
      try {
        ...
        setQuestoes(prev => prev.map(x => (x.numero === q.numero ? { ...x, imagemDataUrl: url } : x)));
      } catch (e) {
  ...
  function removerImagem(numero: number) {
    setQuestoes(prev => prev.map(x => (x.numero === numero ? { ...x, imagemDataUrl: undefined } : x)));
  }
