...
=== PRECISÃO FACTUAL (OBRIGATÓRIO — já causou erro grave antes) ===
Se uma questão ou alternativa fizer qualquer afirmação técnica, numérica ou comparativa que NÃO esteja literalmente escrita nas falas/cena da charge (ex: medidas de quadra/gol, comparação de tamanho com outro esporte, nome oficial de uma regra, distância exata), essa afirmação PRECISA ser real e tecnicamente correta de verdade — nunca aproximada, inventada ou "plausível". Se você não tem certeza absoluta de um dado técnico/numérico, NÃO o use como base de uma alternativa; prefira uma questão que se apoie só no que a própria charge mostrou/disse. Regra dura: para toda questão objetiva, confira mentalmente CADA alternativa uma por uma antes de finalizar — a marcada como "correta" tem que estar 100% correta sem nenhuma ressalva, e TODAS as outras três têm que estar claramente e inequivocamente erradas (nunca "parcialmente certas", "certas em outro contexto" ou uma questão de interpretação) — se não conseguir garantir isso com certeza, reformule a questão inteira (troque o que está sendo perguntado) em vez de manter alternativas duvidosas.
