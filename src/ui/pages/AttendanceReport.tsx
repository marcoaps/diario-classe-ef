...
export function AttendanceReport() {
  ...
  React.useEffect(() => {
    if (!turmaId && uniqueClassRooms.length > 0) {
      setTurmaId(uniqueClassRooms[0].name);
    }
    ...
    <div className="flex flex-col gap-6 font-sans animate-in fade-in pb-24 bg-[#f5f7fb] min-h-screen p-4 md:p-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-2 py-2 px-3 rounded-lg font-semibold text-xs bg-emerald-600 text-white hover:bg-emerald-700 active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
          >
          ...
          <button
            ...
            onClick={handlePDF}
            disabled={loading || alunos.length === 0}
            className="flex items-center gap-2 py-2 px-3 rounded-lg font-semibold text-xs bg-rose-600 text-white hover:bg-rose-700 active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
          >
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-5">
            ...
            <div className="grid grid-cols-4 gap-2">
              {BIMESTRES.map((b) => (
                <button
                  ...
                  className={cn(
                    'py-2 rounded-xl text-sm font-bold border transition-all active:scale-95',
                    bimestre === b
        ...
        <p className="text-[11px] text-gray-400 mt-3">
          Período: {formatarBR(periodo.inicio)} a {formatarBR(periodo.fim)} • {PONTOS_MAXIMOS} aulas •{' '}
          {PONTOS_MAXIMOS * 0.5} pts máx
        </p>
      </div>
