...
import { Loader2, BarChart3, Trash2, ClipboardList } from 'lucide-react';
...
export function AttendanceReport() {
  ...
  const alunosCriticos = useMemo(() =>
    relatorio.filter(a => a.frequencia < 50), [relatorio]);

  const loadRelatorio = async () => {
    setLoading(true);
    try {
      const dados = await buscarRelatorioFrequencia(turmaId, dataInicio || undefined, dataFim || undefined);
      setRelatorio(dados);
    } catch (err) {
  ...
  useEffect(() => { loadRelatorio(); }, [turmaId, dataInicio, dataFim]);

  async function criarAvaliacaoRecuperacao() {
    if (alunosCriticos.length === 0) return;
    if (turmaId === 'ALL') {
      alert('Selecione uma turma específica para criar a avaliação de recuperação.');
      return;
    }

    setCriandoAvaliacao(true);
    try {
      // Cria avaliação de recuperação
      ...
      const { data: avaliacao, error } = await supabase
        ...
        .insert({
          ...
          descricao: `Alunos críticos (frequência < 50%) — ${alunosCriticos.length} alunos`,
          turma_id: turmaId,
          num_questoes: 10,
          gabarito,
          valor_questao: 1.0,
          alunos_ids: alunosCriticos.map(a => a.id),
        })
        .select()
        .single();
      ...
      navigate(`/avaliacoes/folha/${avaliacao.id}?criticos=${alunosCriticos.map(a => a.id).join(',')}`);
    } catch (e: any) {
      setCriandoAvaliacao(false);
    ...
    <div className="flex flex-col gap-6 font-sans animate-in fade-in pb-24 bg-[#f5f7fb] min-h-screen p-4 md:p-6">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          disabled={loading}
          className="flex items-center justify-center gap-2 py-2 px-3 rounded-lg font-semibold text-xs border border-red-200 text-red-600 hover:bg-red-50 active:scale-95 transition-all disabled:opacity-50"
        >
      ...
      {alunosCriticos.length > 0 && turmaId !== 'ALL' && (
        <div className="bg-red-50 border border-red-200 rounded-2xl p-4 flex items-center justify-between gap-3">
          <div>
            <p className="text-sm font-bold text-red-700">
              {alunosCriticos.length} aluno{alunosCriticos.length > 1 ? 's' : ''} crítico{alunosCriticos.length > 1 ? 's' : ''} em {turmaId}
            </p>
            <p className="text-xs text-red-500 mt-0.5">Frequência abaixo de 50% — precisam fazer prova</p>
          </div>
          <button
            onClick={criarAvaliacaoRecuperacao}
            disabled={criandoAvaliacao}
            className="flex items-center gap-2 px-4 py-2 bg-red-600 text-white rounded-xl text-xs font-bold whitespace-nowrap disabled:opacity-60"
          >
            <ClipboardList className="w-4 h-4" />
            {criandoAvaliacao ? 'Criando...' : 'Criar prova'}
          </button>
        </div>
      )}
            ...
            <div className="hidden md:block w-full overflow-x-auto rounded-3xl border border-gray-100 shadow-sm bg-white">
              <table className="w-full text-sm text-left">
                <thead className="bg-gray-100 text-gray-600 text-xs uppercase font-bold tracking-wider">
                  <tr>
                    <th className="px-5 py-4 min-w-[200px]">Aluno</th>
                    <th className="px-4 py-4 text-center">Turma</th>
                ...
                <tbody className="divide-y divide-gray-100">
                  {relatorio.map((aluno: any) => (
                    <tr key={aluno.id} className="hover:bg-cyan-50/50 transition-colors odd:bg-white even:bg-gray-50/30">
                      ...
                      <td className="px-4 py-4 text-center text-red-500 font-semibold">{aluno.faltas}</td>
                      <td className="px-4 py-4 text-center font-bold text-gray-900">{aluno.frequencia}%</td>
              ...
              {relatorio.map((aluno: any) => (
                <div key={aluno.id} className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col gap-3">
                  ...
                  <div className="flex justify-between text-xs font-medium text-gray-600">
                    ...
                    <span className="text-red-500 font-semibold">Faltas: {aluno.faltas}</span>
                  </div>
                  <div className="flex justify-between items-center mt-1 pt-2 border-t border-gray-50">
                    <span className="font-bold text-lg text-gray-900">{aluno.frequencia}%</span>
                    <span className={cn(
                      "px-2 py-1 rounded-full text-[10px] font-bold text-white",
                      aluno.frequencia >= 50 ? 'bg-green-500' : 'bg-red-500'
                    )}>
