...
const DISCIPLINAS = [
  'L. Portuguesa', 'Arte', 'E. FÃ­sica', 'L. Inglesa', 'L. Espanhola',
  'MatemÃ¡tica', 'CiÃªncias', 'Geografia', 'HistÃ³ria', 'ReligiÃ£o',
] as const;
...
const GRUPOS_INICIAIS: GrupoData[] = [
  {
    nome: '6Âº ANO',
    turmas: [
      { serie: '6Âº A', total: 36, trans: '-', freq: 36, reprovados: makeVazio(), recuperados: makeVazio() },
      { serie: '6Âº B', total: 35, trans: '-', freq: 35, reprovados: makeVazio(), recuperados: makeVazio() },
      { serie: '6Âº C', total: 37, trans: '1', freq: 36, reprovados: makeVazio(), recuperados: makeVazio() },
      { serie: '6Âº D', total: 32, trans: '-', freq: 32, reprovados: makeVazio(), recuperados: makeVazio() },
      { serie: '6Âº E', total: 32, trans: '-', freq: 32, reprovados: makeVazio(), recuperados: makeVazio() },
      { serie: '6Âº F', total: 25, trans: '1', freq: 24, reprovados: makeVazio(), recuperados: makeVazio() },
    ],
  },
  {
    nome: '7Âº ANO',
    turmas: [
      { serie: '7Âº A', total: 35, trans: '-', freq: 35, reprovados: makeVazio(), recuperados: makeVazio() },
      { serie: '7Âº B', total: 35, trans: '-', freq: 34, reprovados: makeVazio(), recuperados: makeVazio() },
      { serie: '7Âº C', total: 35, trans: '-', freq: 35, reprovados: makeVazio(), recuperados: makeVazio() },
      { serie: '7Âº D', total: 26, trans: '-', freq: 26, reprovados: makeVazio(), recuperados: makeVazio() },
      { serie: '7Âº E', total: 26, trans: '-', freq: 26, reprovados: makeVazio(), recuperados: makeVazio() },
      { serie: '7Âº F', total: 26, trans: '-', freq: 26, reprovados: makeVazio(), recuperados: makeVazio() },
    ],
  },
  {
    nome: '8Âº ANO',
    turmas: [
      { serie: '8Âº A', total: 36, trans: '-',  freq: 36, reprovados: makeVazio(), recuperados: makeVazio() },
      { serie: '8Âº B', total: 36, trans: '-',  freq: 36, reprovados: makeVazio(), recuperados: makeVazio() },
      { serie: '8Âº C', total: 34, trans: '+1', freq: 35, reprovados: makeVazio(), recuperados: makeVazio() },
      { serie: '8Âº D', total: 30, trans: '-',  freq: 30, reprovados: makeVazio(), recuperados: makeVazio() },
      { serie: '8Âº E', total: 25, trans: '+3', freq: 28, reprovados: makeVazio(), recuperados: makeVazio() },
      { serie: '8Âº F', total: 28, trans: '-',  freq: 28, reprovados: makeVazio(), recuperados: makeVazio() },
    ],
  },
  {
    nome: '9Âº ANO',
    turmas: [
      { serie: '9Âº A', total: 34, trans: '-', freq: 34, reprovados: makeVazio(), recuperados: makeVazio() },
      { serie: '9Âº B', total: 32, trans: '-', freq: 32, reprovados: makeVazio(), recuperados: makeVazio() },
      { serie: '9Âº C', total: 32, trans: '-', freq: 32, reprovados: makeVazio(), recuperados: makeVazio() },
      { serie: '9Âº D', total: 34, trans: '-', freq: 34, reprovados: makeVazio(), recuperados: makeVazio() },
      { serie: '9Âº E', total: 34, trans: '2', freq: 32, reprovados: makeVazio(), recuperados: makeVazio() },
    ],
  },
];

const BIMESTRES = ['1Âº Bimestre', '2Âº Bimestre', '3Âº Bimestre', '4Âº Bimestre'];
...
// â”€â”€ Tela de bloqueio para celular â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  return (
    <div style={{
      ...
      <h2 style={{ color: '#e8eaf6', fontSize: 22, fontWeight: 700, marginBottom: 12 }}>
        DisponÃ­vel apenas no computador
      </h2>
      <p style={{ color: '#90caf9', fontSize: 15, lineHeight: 1.6, maxWidth: 320, marginBottom: 24 }}>
        A pÃ¡gina de <strong style={{ color: '#e8eaf6' }}>Resultado do Bimestre</strong> contÃ©m
        uma tabela extensa com todas as disciplinas e turmas. Para melhor visualizaÃ§Ã£o
        e ediÃ§Ã£o, acesse pelo computador.
      </p>
      <div style={{
        background: '#162032',
        ...
        <div>ðŸŒ <strong style={{ color: '#e8eaf6' }}>www.diarioiop.com.br</strong></div>
        <div style={{ marginTop: 6 }}>ðŸ“‚ Menu â†’ <strong style={{ color: '#e8eaf6' }}>Rendimento</strong></div>
      </div>
    </div>
  );
}
...
export function RendimentoBimestre() {
  ...
  // â”€â”€ Detecta mobile â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  useEffect(() => {
    const check = () => {
  ...
  const resumoSerie = (turmas: TurmaData[]) => {
    ...
    const pct = totalApr + totalRep > 0
      ? Math.round(totalApr / (totalApr + totalRep) * 100) : 100;
  ...
  const exportarExcel = () => {
    ...
    try {
      ...
      const dados: any[][] = [
        ...
        [`Resultado do ${BIMESTRES[bimestre]} â€“ ENSINO FUNDAMENTAL 2026`],
        [],
        ['Ano/SÃ©rie', 'Total', 'Trans.', 'Freq.', ...DISCIPLINAS.flatMap(d => [d + ' Rep', d + ' Apr'])],
      ];
    ...
    <div style={{ minHeight: '100vh', background: '#0d1b2a', color: '#e8eaf6', fontFamily: 'Arial, sans-serif' }}>
      ...
      <div style={{ background: '#0a3055', padding: '16px 24px', borderBottom: '2px solid #1565c0' }}>
        ...
        <p style={{ margin: '4px 0 0', fontSize: 13, color: '#90caf9' }}>
          Resultado do {BIMESTRES[bimestre]} â€“ Ensino Fundamental â€“ 2026
        </p>
      </div>
      ...
      <div style={{ background: '#162032', padding: '12px 24px', display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap', borderBottom: '1px solid #1e3a5f' }}>
        ...
        <button onClick={exportarExcel} disabled={exportando}
          ...
          ðŸ“Š {exportando ? 'Exportando...' : 'Exportar Excel'}
        </button>
      </div>
      ...
      <div style={{ padding: '0 24px 40px', overflowX: 'auto' }}>
              ...
              return (
                <React.Fragment key={grupo.nome}>
                      ...
                      <tr style={{ background: ti % 2 === 0 ? '#111e2e' : '#0f1a28' }}>
                        ...
                        {DISCIPLINAS.flatMap(d => {
                          ...
                          return [
                            ...
                            <td key={d + 'a'} style={{ ...tdStyle, color: '#66bb6a', fontWeight: 700 }}>{apr}</td>,
                          ];
                        })}
                      </tr>
                  ...
                  <tr style={{ background: '#3b0000' }}>
                    <td colSpan={4} style={{ ...tdStyle, color: '#ef5350', fontWeight: 700, paddingLeft: 12 }}>Total Reprovados</td>
                    {DISCIPLINAS.flatMap(d => {
                  ...
                  <tr style={{ background: '#1a0a00' }}>
                    <td colSpan={4} style={{ ...tdStyle, color: '#ffa726', fontWeight: 700, paddingLeft: 12 }}>Total Recuperados</td>
                    {DISCIPLINAS.flatMap(d => {
            ...
            {!abaAtiva && (
              <>
                <tr style={{ background: '#1a0000' }}>
                  <td colSpan={4} style={{ ...tdStyle, color: '#ef5350', fontWeight: 700, fontSize: 13, paddingLeft: 12 }}>TOTAL GERAL â€“ Reprovados</td>
                  {DISCIPLINAS.flatMap(d => {
                ...
                <tr style={{ background: '#001a00' }}>
                  <td colSpan={4} style={{ ...tdStyle, color: '#ffa726', fontWeight: 700, fontSize: 13, paddingLeft: 12 }}>TOTAL GERAL â€“ Recuperados</td>
                  {DISCIPLINAS.flatMap(d => {
        ...
        <div style={{ marginTop: 24, background: '#162032', borderRadius: 10, padding: '16px 20px', fontSize: 13, color: '#90caf9', border: '1px solid #1e3a5f' }}>
          <strong style={{ color: '#e8eaf6' }}>ObservaÃ§Ãµes:</strong>
          <ul style={{ margin: '8px 0 0', paddingLeft: 20, lineHeight: 1.8 }}>
            <li>Os alunos transferidos com nota no 1Âº bimestre sÃ£o contados apenas do 2Âº bimestre.</li>
            ...
            <li>CÃ©lulas vermelhas intensas indicam mais de 5 reprovados na disciplina.</li>
          </ul>
        </div>
      </div>
    </div>
...
const thStyle: React.CSSProperties = {
  background: '#0a3055', color: '#e8eaf6', padding: '8px 6px',
  border: '1px solid #1e3a5f', fontWeight: 700, textAlign: 'center',
  whiteSpace: 'nowrap', position: 'sticky', top: 0, zIndex: 2,
};

const tdStyle: React.CSSProperties = {
  padding: '5px 6px', border: '1px solid #1e3a5f',
