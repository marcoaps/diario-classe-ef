...
- CADA "imagemQuery" deste lote precisa descrever uma AÇÃO DIFERENTE das outras questões do mesmo lote, mesmo quando o esporte/tema é o mesmo — nunca repita a mesma cena para duas questões (ex: se uma questão já é sobre arremesso com salto, a próxima questão sobre drible/condução de bola PRECISA descrever a ação de driblar/segurar a bola correndo, não "arremesso" de novo), porque queries parecidos tendem a trazer a MESMA foto do banco de imagens e isso já gerou provas com a foto errada repetida em várias questões.
