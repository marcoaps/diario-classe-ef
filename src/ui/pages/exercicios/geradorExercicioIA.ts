...
=== REGRAS OFICIAIS, NÃO ADAPTADAS (OBRIGATÓRIO) ===
Sempre que a cena envolver uma regra, marcação de quadra/campo ou medida oficial (ex: distância de linha, tamanho de área, tempo de jogo), use exclusivamente as regras oficiais da modalidade ADULTA, segundo a federação internacional correspondente (ex: IHF para handebol, FIVB para vôlei, FIBA para basquete, World Athletics para atletismo) — nunca uma versão "mini"/adaptada para categoria de base ou infantil, mesmo que a turma seja de Ensino Fundamental, a menos que "${params.conteudo}" peça isso explicitamente. Ex: no handebol, a linha de tiro livre fica a 9 metros do gol e a área do gol tem 6 metros — não use as distâncias menores de versões adaptadas (mini-handebol).
